-- Prayer outcomes now live only in the prayer service's schema (prayer.outcome, same database);
-- the app no longer mirrors them into helm_records. Life Hero reads prayer evidence from there.
begin;
  ...
  for v_candidate in
    ...
    candidates as (
      select
        ...
        -- Same reference as when prayers were mirrored into helm_records, so no award repeats.
        'helm:prayerTracking:record:' || outcome.prayer_date::text || '::' || outcome.prayer_name as source_reference,
        (outcome.prayer_date::timestamp + interval '12 hours') at time zone 'UTC' as occurred_at,
        outcome.prayer_date as local_date,
        jsonb_build_object(
          'reason', case outcome.status
            when 'on_time' then 'prayer_completed_on_time'
            else 'prayer_completed_late'
          end,
          'sourceCollection', 'prayerTracking',
          'sourceRecordId', 'record:' || outcome.prayer_date::text || '::' || outcome.prayer_name,
          'prayer', outcome.prayer_name,
          'status', outcome.status
        ) as metadata
      from prayer.outcome as outcome
      where outcome.user_id = v_user_id
        and outcome.status = any(array['on_time', 'late'])
...
comment on function public.sync_life_hero_evidence(date) is
  'Idempotently maps existing database-authoritative life evidence to server-owned Life Hero awards. Prayer evidence comes from prayer.outcome. Raw wealth and product usage never qualify.';
