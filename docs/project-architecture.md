...
The passive sync workflow lives in `src/services/googleCalendarSync/`: pure source ownership and cache reconciliation (`ownership.ts`, `reconcile.ts`), diagnostics and credential-status rules, and `createGoogleCalendarSyncService`, which takes the Google, hosted-credential, diagnostics, clock and Calendar-writer functions as parameters and returns results instead of setting React state. `useGoogleSync` builds it with the live implementations and only holds the published state. Pure Google account-state rules (eligibility, ownership, credential mapping, labels) live in `src/services/googleCalendarAccountState.ts`; `googleCalendarAuthManager.ts` keeps the credential I/O and re-exports them.
...
- The optional Calendar service migration probe checks `GET /api/calendar/health` and `GET /api/calendar/health/database` from the Dashboard. The database probe sends no user data or credentials. The Calendar service skeleton does not call Google Calendar or move OAuth credentials, calendar data, or existing provider behavior; the current Calendar integration remains authoritative.
