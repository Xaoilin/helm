...
- Dashboard is the Night Compass daily operating view: Prayer is the structural first tier, a deterministic static Quran motivation card follows it, Learn and Move are mandatory daily pillars, and one compact due-task route is second-order. Quran cards carry source-reviewed Arabic, exact references, reviewed paraphrase summaries, and Quran.com links; no runtime model writes religious content. The dashboard consumes canonical PrayerContext outcomes and the committed daily-momentum API; it does not rank Prayer as a task or restore the former generic `Up Next`, agenda, habit, goal, achievement, milestone, system-health, or duplicate task panels. The dashboard-focus domain remains available to Debug for diagnostics and model trace inspection, but it no longer owns the primary Dashboard hierarchy. Separate Jafari final deadlines continue to drive canonical On-time/Late outcomes and warnings, while Profile preserves longer prayer history.
...
- Prayer tracking and reminders are specified in `docs/prayer-tracking-and-reminders.md`. `PrayerProvider` owns schedule freshness, canonical outcomes, the shared completion selector, stats, native deadline reconciliation, bounded prayer-opportunity and momentum plans, and Prayer Debug diagnostics. Stable account receipts make new plan attempts idempotent across renderer reloads while permission and process timers stay device-owned. UI, chat, and voice all use the same completion mutation; Lina asks `On time or late?` when status is omitted, and undo restores task, XP, and prayer tracking together.
