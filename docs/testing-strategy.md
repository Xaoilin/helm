...
| Life Hero progress must be permanent and evidence-backed | Pure deterministic progression checks plus transactional local Postgres contracts | Seven versioned stats, no caller or usage-analytics XP, duplicate suppression, source and momentum snapshots, owner-only RLS, monotonic read models, computed conditions, safe legacy provenance, and non-destructive rollback. | Live provider truth or dashboard presentation; those belong to later tickets. |
