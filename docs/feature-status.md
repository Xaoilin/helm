...
| Chat assistant with hosted OpenAI | `real` | Real when Supabase is configured and the `assistant-openai` Edge Function is deployed. Browser builds use the configured hosted GPT-5.4 planner first, then validate and execute locally with grounded IDs and confirmation rules. The live hosted planner is also benchmark-gated on `master` before deployment. |
