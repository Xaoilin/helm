...
| Google Calendar sync | `real` | Events and calendars sync through the Google Calendar API with account-level reconnect states. Passive sync is non-interactive, reconnect-required only comes from confirmed auth failures, and expired cached GIS tokens are treated as transport diagnostics instead of proof that the account disconnected. |
| Google Calendar diagnostics in Debug tab | `real` | `Debug -> Network / APIs` shows HELM auth context, redacted Google token presence/expiry/scope, per-account passive-sync eligibility, and a manual passive auth check that probes Calendar access without mutating sources or events. |
