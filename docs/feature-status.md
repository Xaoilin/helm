...
| Life Hero concept approval artifact | `prototype/concept` | A standalone responsive proof at `concepts/life-hero/index.html` renders a project-bound glTF 2.0 GLB with a 29,410-vertex neutral body, a separately toggleable 6,198-vertex jacket on the same 24-joint skin, and four embedded clips. SVG equipment and reduced-motion/static fallbacks remain. This is not a production rig, wardrobe system, engine decision, or dashboard behavior, and explicit milestone approval is still required before KAN-258. |
