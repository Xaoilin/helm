...
| Google Calendar sync | `real` | Events and calendars sync through the Google Calendar API with account-level reconnect states. Passive sync is non-interactive, Calendar tab navigation is read-only, `Sync` stays passive, reconnect-required only comes from confirmed auth failures, and expired cached GIS tokens are treated as transport diagnostics instead of proof that the account disconnected. Sync is now additive/update-only for passive fetches, so missing items from a partial window or wrong-account response do not delete cached data. |
| Google Calendar diagnostics in Debug tab | `real` | `Debug -> Network / APIs` shows HELM auth context, redacted Google token presence/expiry/scope, per-account passive-sync eligibility, trigger source, blocked reasons, wrong-account detection, skipped destructive removals, and a manual passive auth check that probes Calendar access without mutating sources or events. |
