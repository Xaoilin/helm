...
| Google Calendar diagnostics in Debug tab | `real` | `Debug -> Network / APIs` shows HELM auth context, hosted credential source and health, redacted current access-token expiry, refresh-failure metadata, legacy browser-token migration state, per-account passive-sync eligibility, trigger source, blocked reasons, wrong-account detection, skipped destructive removals, and a manual passive auth check that probes Calendar access without mutating sources or events. |
