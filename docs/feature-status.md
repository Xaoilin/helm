...
| Chat assistant with hosted OpenAI | `real` | Real when Supabase is configured, the `assistant-openai` Edge Function is deployed, and the user has a live signed-in Supabase session token. |
