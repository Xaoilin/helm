...
// ── Single-Object Context (settings, gamification profile, etc.) ──

export interface SingleObjectContextValue<T> {
  value: T;
  loaded: boolean;
...
export function createSingleObjectContext<T>(storeKey: string, defaultValue: T) {
  const Ctx = createContext<SingleObjectContextValue<T> | null>(null);
  ...
  function Provider({ children }: { children: ReactNode }) {
    ...
    return (
      <Ctx.Provider value={{ value, loaded, setValue, update }}>
        {children}
      </Ctx.Provider>
    );
  }
